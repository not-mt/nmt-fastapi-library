...
def test_oauth2_authorization_code_init_defaults():
    """
    Test initialization of OAuth2AuthorizationCode with default parameters.
    """
    ...
    assert scheme.scheme_name == "OAuth2AuthorizationCode"
    assert scheme.auto_error is True

    assert scheme.model.flows.authorizationCode is not None
    assert scheme.model.flows.authorizationCode.authorizationUrl == authorization_url
    assert scheme.model.flows.authorizationCode.tokenUrl == token_url
    assert scheme.model.flows.authorizationCode.scopes == {}


def test_oauth2_authorization_code_init_custom_scheme_name():
    """
    Test initialization of OAuth2AuthorizationCode with a custom scheme name.
    """
    authorization_url = "https://example.com/authorize"
    token_url = "https://example.com/token"
    custom_name = "MyAuthCodeScheme"
    scheme = OAuth2AuthorizationCode(
...
def test_oauth2_authorization_code_init_auto_error_false():
    """
    Test initialization of OAuth2AuthorizationCode with auto_error set to False.
    """
    scheme = OAuth2AuthorizationCode(
...
def test_oauth2_authorization_code_flows_configuration():
    """
    Test that flows are properly configured for authorization code.
    """
    authorization_url = "https://example.com/authorize"
    token_url = "https://example.com/token"
    scheme = OAuth2AuthorizationCode(
    ...
    assert flows.authorizationCode.authorizationUrl == authorization_url
    assert flows.authorizationCode.tokenUrl == token_url
    assert flows.authorizationCode.scopes == {}
    assert flows.clientCredentials is None
    assert flows.implicit is None
    assert flows.password is None


def test_oauth2_authorization_code_repr():
    """
    Test string representation of the authorization code scheme.
    """
    authorization_url = "https://example.com/authorize"
    token_url = "https://example.com/token"
    scheme = OAuth2AuthorizationCode(
    ...
    expected_repr = (
        ...
        f"authorizationUrl='{authorization_url}', "
